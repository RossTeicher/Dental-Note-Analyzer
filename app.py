...
st.title("🦷 Complete Dental AI System")
...
language = st.selectbox("Language for Consent", list(lang_map.keys()))
...
# Animation script
st.markdown("## 🎬 Generate Animation Script")
if st.button("Create Animation Script"):
    try:
        anim_prompt = f"Create an animation storyboard in {language} for a dental patient undergoing: {procedure}. Use plain, empathetic language."
        anim_response = client.chat.completions.create(
            model="gpt-4o",
            messages=[{"role": "user", "content": anim_prompt}]
        )
        st.text_area("📽️ Animation Script", anim_response.choices[0].message.content, height=300)
    except Exception as e:
